...
class MainWindow(Tk):
    ...
    def create_label(self, frame, label, Object="",ligne=1, extend=False):
        if extend:
            ...
            text = Text(frame,font=("Arial",12),state=NORMAL, spacing3=10)
            label.pack(padx=5, pady=5, anchor=W)
            text.pack(padx=5, pady=5, fill=BOTH, expand=True)
            text.insert('1.0', Object)
        elif ligne > 1:
            label = Label(frame, text=label, wrap=None)
            text = Text(frame, height=ligne, state=NORMAL)
            label.pack(padx=5, pady=5, anchor=W)
            text.pack(padx=5, pady=5, fill=X)
            text.insert('1.0', Object)
        else:
            label = Label(frame, text=label)
            text = Entry(frame, state=NORMAL)
            label.pack(padx=5, pady=5, anchor=W)
            text.pack(padx=5, pady=5, fill=X)
    ...
    def create_letter(self):
        ...
        if not is_okay:
            self.show_dialog("Génération","Une erreur est survenue lors de la sauvegarde de la lettre", type="error")  
    ...
    def file_manager(self, type, name, extension,):
        if type == "open":
            ...
            f = filedialog.asksaveasfilename(initialfile=name,defaultextension=extension ,filetypes=[(extension, f".{extension}")])
        return f
